...
summary(gpsdf$date)
summary(TDRm$begdesc)

TDRm<-subset(TDRm,begdesc<max(gpsdf$date))
TDRm<-subset(TDRm,begdesc>min(gpsdf$date))

# Agrate interpolated location
track_interpolated <- data.frame() 

# for(i in 1:length(move_track@idData$burst)) { # Le pido que pase por cada viaje
i = 1
track_sel <- move_track[[i]]

# name <- track_sel@idData$burst # Guarda el nombre
t_interp <- interpolateTime(move_track, time=Dives_HP$begdesc, spaceMethod='euclidean') # Aca interpola: calcula las posiciones para los tiempos que faltan
...
t_interp_df <- as.data.frame (t_interp) # Datos interpolados
t_record_df <- as.data.frame (move_track) # Datos observados

t_interp_df <- rbind (t_interp_df, t_record_df) %>%  # Junta ambos
  arrange (timestamps)
